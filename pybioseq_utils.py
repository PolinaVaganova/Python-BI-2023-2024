...
class AminoAcidSequence(BiologicalSequence, ABC):
    ...
    def __init__(self, seq: str, code: int = 1
        ...
        if self.code == 1:
            ...
            raise ValueError(f'Invalid specified encoding: {self.code}. Available values: 1, 3.')
    ...
    def is_correct(self):
        ...
        for residue in set(self.seq):
            if residue.upper() not in self.residues_names:
                raise AminoAcidNotFoundError(f'Sequence contains invalid aminoacid: {residue}')
        return 'Your sequence is correct.'
...
def run_fastq_filtering(input_path: str, output_filename: str = None, gc_bounds: Union[tuple, float, int] = (0, 100),
